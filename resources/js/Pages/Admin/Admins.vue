...
const openEditModal = (admin) => {
    editingUserId.value = admin.id;
    editForm.name = admin.name;
    editForm.email = admin.email;
    editForm.role = 'admin';
    editForm.tahun_ajaran = admin.tahun_ajaran || '';
    editForm.password = '';
...
const confirmDeleteAdmin = () => {
    ...
    router.delete(route('admin.users.destroy', adminToDelete.value.id), {
        onSuccess: () => {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            Manajemen: <span class="text-gradient-gy capitalize">Data Admin</span>
        </template>
        ...
        <div class="space-y-6 sm:space-y-10">
            ...
            <div class="flex flex-col gap-4">
                <div>
                    <h2 class="text-2xl font-black text-gray-800 tracking-tighter">Daftar Admin</h2>
                    <p class="text-sm text-gray-500 mt-1">Kelola data administrator sistem.</p>
                </div>
                ...
                <div class="flex flex-wrap gap-2 items-center">
                    <div class="relative">
                        ...
                        <input v-model="searchQuery" type="text" placeholder="Cari admin..." class="pl-10 pr-4 py-3 border border-gray-200 rounded-2xl text-sm focus:ring-green-500 focus:border-green-500 w-full sm:w-64 transition-all shadow-sm">
                    </div>
            ...
            <!-- Empty State -->
            <div v-if="filteredAdmins.length === 0" class="p-12 text-center bg-white/50 backdrop-blur-sm rounded-[2rem] border border-green-200 border-dashed">
                ...
                <p class="text-gray-500 mt-2">Admin tidak ditemukan atau belum ditambahkan.</p>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100/50">
                            <tr v-for="(admin, index) in filteredAdmins" :key="admin.id" class="hover:bg-green-50/30 transition-colors">
                                <td class="p-4 text-sm font-medium text-gray-500">{{ props.admins.from + index }}</td>
                                <td class="p-4 text-sm font-bold text-gray-800">{{ admin.name }}</td>
                                <td class="p-4 text-sm font-medium text-gray-600">{{ admin.tahun_ajaran || '-' }}</td>
                                <td class="p-4 text-sm font-medium text-gray-600">{{ admin.email }}</td>
                                <td class="p-4 flex items-center justify-center gap-2">
                                    <button @click="openEditModal(admin)" class="p-2 text-yellow-600 hover:bg-yellow-50 rounded-lg transition-colors" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                        </svg>
                                    </button>
                                    <button @click="deleteAdmin(admin)" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus" v-if="admin.id !== $page.props.auth.user.id">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <Modal :show="isEditModalOpen" @close="closeEditModal" maxWidth="lg">
            <div class="p-6">
                <h2 class="text-xl font-black text-gray-800 uppercase tracking-tighter mb-4">Edit Data Admin</h2>
        ...
        <Modal :show="showConfirmDeleteModal" @close="showConfirmDeleteModal = false" maxWidth="sm">
            <div class="p-6 text-left">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="text-sm font-black text-gray-800 uppercase tracking-wider">Hapus Admin</h3>
                </div>
                
                <p class="text-xs font-medium text-gray-500 mb-6 leading-relaxed">
                    Apakah Anda yakin ingin menghapus data admin <span class="font-bold text-gray-800">{{ adminToDelete?.name }}</span>? Tindakan ini tidak dapat dibatalkan.
                </p>
